...
/**
 * This class implements a Stack using two different implementations. Stack is used with a regular
 * array and Stack2 uses an ArrayList.
 *
...
/**
 * This is an ArrayList Implementation of stack, Where size is not a problem we can extend the stack
 * as much as we want.
 *
 * @author Unknown
 */
...
public class Matrix {
  ...
  public Matrix(int[][] pData) {
    ...
    if (pData.length != 0) {
      ...
      for (int i = 0; i < pData.length; i++)
        for (int j = 0; j < pData[0].length; j++) newData[i][j] = pData[i][j];
  ...
  /**
   * Returns this matrix scaled by a factor. That is, computes sA where s is a constant and A is a
   * matrix (this object).
   *
  ...
  public Matrix scale(int scalar) {
    ...
    for (int i = 0; i < this.getRows(); ++i)
      for (int j = 0; j < this.getColumns(); ++j) newData[i][j] = this.data[i][j] * scalar;

    return new Matrix(newData);
  }

  /**
   * Returns this matrix divided by a factor. That is, computes sA where s is a constant and A is a
   * matrix (this object).
   *
   ...
   * <p>[ a b c ] ... [ x y z ] ... [ i j k ] ... ...
   *
   * @return Matrix as String TODO: Work formatting for different digit sizes
   */
  ...
  public Matrix transpose() {
    ...
    for (int i = 0; i < this.getColumns(); ++i)
      for (int j = 0; j < this.getRows(); ++j) newData[i][j] = this.data[j][i];
